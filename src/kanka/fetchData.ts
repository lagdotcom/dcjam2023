...
const fixName = (s: string) =>
  s.replace(" of ", " Of ").replace(/[^a-zA-Z]/g, "");
const byName = (a: { name: string }, b: { name: string }) =>
  a.name.localeCompare(b.name);

function fixProse(s: string) {
  let build = s;
  let working = true;

  while (working) {
    const i = build.indexOf("<");
    if (i >= 0) {
      const j = build.indexOf(" ", i);
      const k = build.indexOf(">", i);
      const tag = build.slice(i + 1, j);

      let replace = "";
      if (tag === "p" || tag === "/p") replace = "\n";

      build = build.slice(0, i) + replace + build.slice(k + 1);
    } else working = false;
  }

  return build.trim().replace(/&nbsp;/g, " ");
}
...
const abilityNameGetter =
  ...
  (ea?: EntityAbility) => {
    ...
    return fixName(ability.name);
  };

const itemNameGetter =
  (context: string, items: ItemWithRelated[]) => (iv?: Inventory) => {
    const item = items.find((i) => i.id === iv?.item_id);
    if (!item) throw new Error(`Could not find item for ${context}`);
    return fixName(item.name);
  };

function tsAction(ability: AbilityWithRelated) {
  const name = fixName(ability.name);
...
function tsPC(character: CharacterWithRelated, items: ItemWithRelated[]) {
  const attr = new AttributeBag(
    `character ${character.name}`,
    character.attributes,
  );
  const getItemName = itemNameGetter(`character ${character.name}`, items);

  const lore = fixProse(character.entry_parsed);

  return `"${character.title}": {
    name: "${character.name}",
    lore: \`${lore}\`,
    deathQuote: \`${attr.req("Death Quote")}\`,
    hp: ${attr.req("hp")},
    sp: ${attr.req("sp")},
    determination: ${attr.req("determination")},
    camaraderie: ${attr.req("camaraderie")},
    spirit: ${attr.req("spirit")},
    items: [${character.inventory
      .map(getItemName)
      .map((name) => `items.${name}`)
      .join(",")}],
    skill: "${attr.req("Skill")}",
  },`;
}

function validPC(character: CharacterWithRelated) {
  const attr = new AttributeBag("", character.attributes);
  if (
  ...
  console.log(`-- skipping character: ${character.name}`);
  return false;
}

function tsClasses({ characters, items }: KankaData) {
  return `${header}
  ...
  ${characters
    ...
    .sort(
      (a, b) =>
        ClassNames.indexOf(a.title as ClassName) -
        ClassNames.indexOf(b.title as ClassName),
    )
    .map((ch) => tsPC(ch, items))
    .join("\n\n")}
  };
...
function tsEnemy(
  ...
  const name = fixName(creature.name);
...
async function main() {
  ...
  const filenames = await Promise.all([
    ...
    write("./src/classes.ts", tsClasses(data)),
